from cash_money.indicators.indicator import Indicator
from cash_money.objects.strategy_params import DataSelector, NumberParam
from cash_money.cmErrors import IndicatorError
from .lineManager import LineManager, Data


class SoloEMA:
    def __init__(self, *, alpha: float = None, period: float = None, smoothing: float = 2):
        self._avg = None
        if alpha is not None:
            self._a = alpha
        elif period is None or smoothing is None:
            raise IndicatorError('Alpha or period and smoothing not supplied')
        else:
            self._a = smoothing / (period + 1)

        self._ia = 1 - self._a

    def next(self, data) -> float:
        if self._avg is None:
            self._avg = data
        else:
            self._avg = data * self._a + self._avg * self._ia
        return self._avg

    def getValue(self) -> float:
        return self._avg


_EMA = 'EMA'


class EMA(Indicator):
    """
    Exponential Moving Average
    """
    params = [NumberParam('period', "Period", int, 5),
              NumberParam('smoothing', "Smoothing", float, 2.0),
              DataSelector()]
    outputs = [_EMA]

    def __init__(self, name: str, lineManager: LineManager, period: int, smoothing: int, data: Data):
        super().__init__(name)
        self._data = data.requestLine(name, lineManager)
        self._ema = SoloEMA(smoothing=smoothing, period=period)
        self.avg = lineManager.registerLine(name, _EMA)

    def update(self):
        out = self._ema.next(self._data())
        self.avg.add(out)

    def setupTime(self) -> int:
        return 2
